...
const quinchoComuniBookingResourceId = 150
    ...
    <table id="bookingsformControls">
      ...
      <tr>
        ...
        <td valign="middle" width="100" colspan="2">
          <select id="bookingsFormPlace"  onchange="bookingResourceSelectedHOME(this)">
            <option value="0" selected="selected">SALON HOUSE</option>
            <option value="1" >QUINCHO</option>
            <option value="2" >QUINCHO COMUNITARIO</option>
          </select>
        </td>
      </tr>
      <tr>
...
Sábados y Visp. de Feriados de 19:00 hs a 03:00 hs, valor $ 2.000.-<br>

 <strong>-QUINCHO COMUNITARIO-FAMILIAR</strong><br>


Lunes a Viernes de 11:00 hs a 16:00 hs, valor $ 500.-<br>
Sábados y Domingos de 11:00 hs a 17:00 hs, valor $ 500.-<br>
Lunes a Domingos de 21:00 A 24:00 hs, valor $ 500.-
      </div>
...
function renderBookingsStatus(resourceId)
  if resourceId = quinchoBookingResourceId or resourceId = quinchoComuniBookingResourceId or resourceId=HouseBookingResourceId then
...
function renderClubHouseBookingsStatus(resourceId)
 ...
 <section class='headerTable'>
    ...
    dim placeId: placeId=2
    if resourceId =  HouseBookingResourceId then
      placeId = 0
    elseIf resourceId = quinchoBookingResourceId then
      placeId=1
    end if
...
function bookingsSendRequest(bookingDate, placeId, turnId)
  ...
  elseif placeid=1 then
    ...
    resourceId = quinchoComuniBookingResourceId
    resourceName = "QuinchoComunitario"
  end if 
  
    dim sqlDate: sqlDate = sqlValue(dbDatePrefix & bookingDate)
    
    if resourceId =150 then 
      dbGetData("SELECT CAST(CASE WHEN DATEDIFF(D, (SELECT TOP 1 FECHA FROM HOY), " & sqlDate & ") >1 AND DATEDIFF(D, (SELECT TOP 1 FECHA FROM HOY), " & sqlDate & ") <=4 THEN 1 ELSE 0 END AS BIT)")
    else
      dbGetData("SELECT CAST(CASE WHEN DATEDIFF(D, (SELECT TOP 1 FECHA FROM HOY), " & sqlDate & ") >7 AND DATEDIFF(D, (SELECT TOP 1 FECHA FROM HOY), " & sqlDate & ") <=90 THEN 1 ELSE 0 END AS BIT)")
    end if
    ...
    if dateOK then
      ...
      if resourceId =150 then 
        ...
        if uCase(rs(0)) = "DISPONIBLE" then
          dbGetData("SELECT dbo.ESTADO_TURNO_RESERVA(150, " & sqlDate & ", " & turnStart & ", " & turnDuration & ")")
        end if 
      else
        dbGetData("SELECT dbo.ESTADO_TURNO_RESERVA(" & resourceId & ", " & sqlDate & ", " & turnStart & ", " & turnDuration & ")")
      end if
      ...
      if uCase(status) = "DISPONIBLE" then
        ...
        sendMail "Reservas", "intendencia.glorietas@avn-nordelta.com","Familia: " &usrName& " - Solicitud de reserva", message, "Familia " & usrName, usrEmail
      ...
      JSONAddOpFailed
      if  resourceId =150 then
        JSONAddMessage "Las reservas del " & resourceName & " deben solicitarse con una anticipación minima de 2 días y dentro de los siguientes 4 dias."
      else
        JSONAddMessage "Las reservas del " & resourceName & " deben solicitarse con una anticipación minima de 7 días y dentro de los siguientes 90 dias."
      end if 
