...
function EnviarAlta
 if dbGetData("SELECT NOMBRE, UNIDAD, EMAIL,CLAVE FROM VECINOS WHERE ID=" & recordId) then
    dim mensaje : mensaje= "<h3>Bienvenido a la web de Vecinos de Glorietas, le confirmamos el ALTA, su CONTRASEÑA PROVISORIA de acceso es " & rs("CLAVE").value & ",  sírvase ingresar en el sistema nuevamente y modifique su clave personal. "&_
...
"Equipo Vecinos de Las Glorietas </h2>"

      dim result: result = sendCDOMail(rs("NOMBRE").value, rs("EMAIL").value,"Alta de las glorietas",mensaje,"Equipo de las glorietas",mainEmailAddress)
